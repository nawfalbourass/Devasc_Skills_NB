...
req = requests.request('GET', token_req_url, auth=auth, verify=False)
token = req.json()['Token']

# INVENTORY REQUEST
req_url = DNAC_scheme+DNAC_authority+DNAC_port+DNAC_path
headers = {'X-AUTH-TOKEN': token}
resp_devices = requests.request('https://...', req_url, headers=headers, verify=False)
...
for device in resp_devices_json['response']:
    if device['type'] != None:
        ...
        print('Reachability: ' + device['reachability'])